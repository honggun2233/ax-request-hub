...
const NAV: NavGroup[] = [
  ...
  },
  {
    title: "내 과제",
    items: [
      { href: "/chat",        label: "과제 신청",  icon: Plus },
  ...
  },
  {
    ...
    roles: ["AX_TEAM"],
    items: [
      { href: "/dashboard",     label: "과제 파이프라인", icon: BarChart3 },
      { href: "/registry",      label: "에이전트",        icon: Cpu },
      { href: "/council",       label: "협의회",          icon: Gavel },
        ...
        {visible.map((group, gi) => {
          ...
          return (
            <div key={gi}>
              ...
              <div className="mb-1">
                {group.title && (
                  <p className="px-2 pb-1 pt-3 text-[10px] font-semibold text-gray-400 uppercase tracking-wider">
                    {group.title}
                  </p>
                )}
        ...
        <div className="p-3 border-t bg-gray-50">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className={cn("text-[10px] px-1.5 py-0.5 rounded font-medium", LEVEL_BADGE[level] ?? LEVEL_BADGE.L0)}>
                {level}
              </span>
            </div>
          </div>
          <button
            onClick={() => signOut()}
            className="flex items-center gap-1.5 w-full text-xs text-gray-500 hover:text-red-500 py-1 transition-colors"
          >
