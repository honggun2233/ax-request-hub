...
| 역할 | 설명 | 주요 접근 범위 |
|------|------|---------------|
| `EMPLOYEE` | 일반 직원 | /submit, /chat, /me/*, /skills, /docs, /data/catalog |
| `DEPT_HEAD` | 부서장 | EMPLOYEE + /dept/tools (AI 도구 배정·위임) |
| `DATA_PLATFORM` | 데이터플랫폼팀 | EMPLOYEE + /dp/requests (데이터 신청 검토·제공) |
| `EXECUTIVE` | C레벨 임원 | /executive (읽기 전용 경영진 대시보드) |
| `ADMIN` | AX팀 | 전체 페이지 (/admin/*, /dashboard, /governance, /registry, /council) |
