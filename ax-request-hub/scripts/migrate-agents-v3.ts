...
const FIELD_MAP = {
  ...
  registryStageField: "lifecycleStage", // AgentRegistry.lifecycleStage: "GATE1" | "GATE2" | "GATE3"
  // 기존 Agent(폐기 관리)에서 라이프사이클 상태를 담던 필드명
  legacyStatusField: "status", // Agent.status: "ACTIVE" | "SUSPENDED" | "DEPRECATED" | "RETIRED"
...
async function main() {
  ...
  // 멱등성 설계 (재실행 안전) — $transaction 미사용으로 SQLite 락 충돌 방지
  const legacyAgents: any[] = await (prisma as any).agent.findMany({
    include: { kpiRecords: true }, // AgentKpiRecord 관계명
  });

  // 상용 이관 건이 있으면 소급 승인 협의회를 루프 진입 전에 미리 생성
  const hasProd = legacyAgents.some((la: any) => PROD_MAP[la[FIELD_MAP.legacyStatusField]]);
  const meetingId = hasProd ? await ensureRetroMeeting() : null;
  ...
  for (const la of legacyAgents) {
    ...
    let reg = await prisma.agentRegistry.findFirst({ where: { name: la.name } });
    const data = {
      phase: mapped.phase,
      devStage: null as string | null,
      prodStatus: mapped.prodStatus,
      prodKpiTarget: la[FIELD_MAP.legacyKpiTargetField] != null
        ? String(la[FIELD_MAP.legacyKpiTargetField])
        : null,
    ...
    if (DRY_RUN) {
      ...
      reg = await prisma.agentRegistry.update({ where: { id: reg.id }, data });
      report.prodMigrated++;
    } else {
      // 레거시 Agent에서 AgentRegistry 신규 생성: 필수 필드 채우기
      reg = await prisma.agentRegistry.create({
        data: {
          agentName: la.name,
          agentKey: `legacy-${la.id}`,
          purpose: la.description || la.name,
    ...
    if (!DRY_RUN && reg && meetingId) {
      const exists = await prisma.councilAgendaItem.findFirst({
    ...
    // AgentKpiRecord 필드: recordMonth, actualValue, targetValue, achieveRate
    for (const k of la.kpiRecords ?? []) {
      if (DRY_RUN) { report.scoresMigrated++; continue; }
      if (!reg) continue;
      const month = k.recordMonth as string;
      const kpiActual = JSON.stringify({ value: k.actualValue, target: k.targetValue });
      await prisma.agentScore.upsert({
        where: { agentId_phase_month: { agentId: reg.id, phase: "PRODUCTION", month } },
        update: {},
        create: {
          agentId: reg.id,
          phase: "PRODUCTION",
          month,
          kpiActual,
          achieveRate: Math.round((k.achieveRate ?? 0) as number),
        },
      });
