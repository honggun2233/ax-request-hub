...
export default function HomePage() {
  ...
  if (!data) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '40vh', color: MUTED, fontSize: 14 }}>
      로딩 중...
    </div>
  )
  ...
  const card: React.CSSProperties = { background: CARD, border: `1px solid ${BDR}`, borderRadius: 4, padding: '18px 20px' }
  ...
  return (
    <div style={{ color: TEXT, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, color: '#EBF0F5', margin: 0 }}>운영 현황</h1>
          <p style={{ fontSize: 13, color: MUTED, marginTop: 4 }}>삼성자산운용 AI 거버넌스 플랫폼</p>
        </div>
        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: MUTED, background: CARD, border: `1px solid ${BDR}`, padding: '6px 12px', borderRadius: 20 }}>
        ...
        {KPIs.map((kpi, i) => {
          ...
          return (
            <div key={kpi.label} style={{ ...card, ...leftStripe(kpi.accent), animationDelay: `${i * 80}ms` }} className="kpi-animate">
              ...
              <p style={{ fontSize: 30, fontWeight: 700, color: '#EBF0F5', fontFamily: 'monospace', lineHeight: 1 }}>{val}</p>
        ...
        <div style={card}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={serviceData} barCategoryGap="35%">
              ...
              <Tooltip contentStyle={{ background: CARD2, border: `1px solid ${BDR}`, borderRadius: 4, fontSize: 12, color: TEXT }} />
              <Bar dataKey="tokens" fill={GOLD} radius={[2, 2, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div style={card}>
            ...
            <thead>
              <tr style={{ background: 'rgba(255,255,255,.03)' }}>
                {['이름', '부서', '레벨', '상태', '신청일'].map(h => (
                  <th key={h} style={{ padding: '8px 16px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: MUTED, textTransform: 'uppercase', letterSpacing: '.08em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(data.recentApplications || []).map((a: any) => (
                <tr key={a.id} style={{ borderTop: `1px solid ${BDR}` }}>
                  ...
                  <td style={{ padding: '9px 16px', fontFamily: 'monospace', color: GOLD, fontWeight: 700 }}>{a.requestedLevel}</td>
                  <td style={{ padding: '9px 16px' }}>
                    <span style={{
                      fontSize: 10, padding: '2px 8px', borderRadius: 2, fontWeight: 600,
                      background: a.status === 'PENDING' ? 'rgba(208,123,58,.15)' : a.status === 'APPROVED' ? 'rgba(91,140,110,.15)' : 'rgba(176,64,64,.15)',
                      color:      a.status === 'PENDING' ? '#FFB74D'              : a.status === 'APPROVED' ? '#7EB88A'              : '#E57373',
                      border: `1px solid ${a.status === 'PENDING' ? 'rgba(208,123,58,.3)' : a.status === 'APPROVED' ? 'rgba(91,140,110,.3)' : 'rgba(176,64,64,.3)'}`,
                    }}>
          ...
          <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {(data.recentProjects || []).map((p: any) => (
                <tr key={p.id} style={{ borderTop: `1px solid ${BDR}` }}>
                  ...
                  <td style={{ padding: '9px 16px', fontFamily: 'monospace', color: GOLD, fontWeight: 700 }}>{p.totalScore ?? '–'}</td>
                  <td style={{ padding: '9px 16px' }}>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 2, fontWeight: 600, background: 'rgba(201,169,110,.12)', color: GOLD, border: `1px solid rgba(201,169,110,.3)` }}>
