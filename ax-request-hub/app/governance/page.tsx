    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {(data.stats || []).slice(0, 3).map((s: any) => (
          <div key={s.entityType} className="bg-white rounded-lg border p-4">
            <p className="text-sm text-[var(--muted)]">{s.entityType}</p>
            <p className="text-2xl font-bold text-gray-800">{typeof s._count === 'object' ? s._count._all ?? 0 : s._count}</p>
          </div>
        ))}
        {(data.stats || []).length === 0 && (
          <div className="col-span-3 bg-white rounded-lg border p-4 text-center text-[var(--muted)] text-sm">
            감사 로그 데이터 없음
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg border">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50"><tr>{['시간', '유형', '액션', '대상 ID', '실행자', '상세'].map(h => <th key={h} className="px-3 py-2 text-left text-xs text-[var(--muted)]">{h}</th>)}</tr></thead>
          <tbody>
            {(data.logs || []).length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--muted)]">로그 없음</td></tr>}
            {(data.logs || []).map((l: any) => (
              <tr key={l.id} className="border-t hover:bg-gray-50">
                <td className="px-3 py-2 text-[var(--muted)] text-xs">{new Date(l.createdAt).toLocaleString('ko-KR')}</td>
                ...
                <td className="px-3 py-2 font-mono text-xs text-[var(--muted)]">{l.entityId.slice(0, 8)}...</td>
                <td className="px-3 py-2 text-[var(--muted)]">{l.actorEmail}</td>
                <td className="px-3 py-2 text-[var(--muted)] truncate max-w-[200px]">{l.detail}</td>
              </tr>
            ))}
          </tbody>
        </table>
