...
```ts
// instrumentation.ts — Next.js 서버 초기화 훅, authorize()보다 먼저 실행됨
export async function register() {
  // 안전장치3: 진짜 부팅 시점 차단 — 로그인 시도를 기다리지 않고 서버 시작 자체를 막음
  if (process.env.NODE_ENV === 'production' && process.env.DEV_BYPASS_USER) {
    throw new Error('DEV_BYPASS_USER가 프로덕션에 설정됨 — 서버 시작 차단')
  }
}
```
...
async authorize(credentials) {
  // ▼ 개발환경 우회 — 이중 안전장치 (프로덕션 차단은 instrumentation.ts가 담당)
...
**3중 방어 (위치 정정)**: (1) `NODE_ENV`와 별도 env var 둘 다 필요 — 배포 스크립트가 실수로 `DEV_BYPASS_USER`만 남겨도 `NODE_ENV=production`이면 안 걸림 (2) 프로덕션에서 이 변수가 살아있으면 **`instrumentation.ts`에서 서버 시작 자체를 막아** 첫 로그인 시도를 기다리지 않고 배포 직후 바로 드러나게 함 — `authorize()` 안에 두면 서버는 정상 기동하고 첫 로그인 시도자가 에러를 맞는 것뿐이라 "조용히 숨어있다가 터지는" 문제가 생김 (3) 활성화 시 콘솔 경고로 로컬 개발자도 인지하게 함.
...
| OO | 프로덕션 차단 어설션이 `authorize()` 안에 있어 "부팅 시 차단"이 아니라 "첫 로그인 시도 시 차단"으로 동작 — 서술과 실제 동작 불일치 | 어설션을 `instrumentation.ts`(Next.js 서버 초기화 훅)로 이동, 서버 시작 자체를 막아 배포 직후 바로 드러나게 정정 |
